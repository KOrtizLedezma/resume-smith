...
def get_company_name():
    path = Path("data/outputs/company_name.txt")
    if not path.exists():
...
def get_position_title():
    path = Path("data/outputs/job_title.txt")
    if not path.exists():
...
if __name__ == "__main__":
    ...
    output_path = Path(f"data/outputs/{company}/{file_name}")
    with open("data/outputs/output_path.txt", "w") as f:
